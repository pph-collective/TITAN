...
def simulation(nreps, save_adjlist_flag, time_range, 
               N_pop, outfile_dir, parameters, rSeed):
   # Check input
   ...
   for num_sim in range(nreps):
      ...
      print "\n\n----------------------------------------------"
      print "\tProcess %5s runs simulation %d/%d\t.:.\tRandom seed: %d"%(pid, num_sim+1, nreps, rSeed)
      MyModel = HIVModel(N = N_pop, tmax = time_range, parameter_dict = parameters, rseed = rSeed)
