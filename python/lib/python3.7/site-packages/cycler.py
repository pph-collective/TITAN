...
class Cycler(object):
    ...
    def change_key(self, old, new):
        ...
        if new in self._keys:
            raise ValueError(
                "Can't replace %s with %s, %s is already a key" % (old, new, new)
            )
        if old not in self._keys:
            raise KeyError(
                "Can't replace %s with %s, %s is not a key" % (old, new, old)
            )
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            trans = self.by_key()
            return reduce(add, (_cycler(k, v[key]) for k, v in six.iteritems(trans)))
        else:
    ...
    def __add__(self, other):
        ...
        if len(self) != len(other):
            raise ValueError(
                "Can only add equal length cycles, "
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Cycler):
            ...
            trans = self.by_key()
            return reduce(add, (_cycler(k, v * other) for k, v in six.iteritems(trans)))
        else:
...
def concat(left, right):
    ...
    if left.keys != right.keys:
        msg = "\n\t".join(
            ["Keys do not match:", "Intersection: {both!r}", "Disjoint: {just_one!r}"]
        ).format(both=left.keys & right.keys, just_one=left.keys ^ right.keys)
