...
def bellman_ford_predecessor_and_distance(G, source, target=None, weight="weight"):
    ...
    dist = _bellman_ford(G, [source], weight, pred=pred, dist=dist, target=target)
    return (pred, dist)


def _bellman_ford(G, source, weight, pred=None, paths=None, dist=None, target=None):
...
def bidirectional_dijkstra(G, source, target, weight="weight"):
    ...
    while fringe[0] and fringe[1]:
        ...
        for w in neighs[dir](v):
            if dir == 0:  # forward
                if G.is_multigraph():
                    minweight = min((dd.get(weight, 1) for k, dd in G[v][w].items()))
                else:
                    minweight = G[v][w].get(weight, 1)
                vwLength = dists[dir][v] + minweight  # G[v][w].get(weight,1)
            else:  # back, must remember to change v,w->w,v
                if G.is_multigraph():
                    minweight = min((dd.get(weight, 1) for k, dd in G[w][v].items()))
                else:
