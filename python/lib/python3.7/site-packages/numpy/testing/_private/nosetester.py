    ...
    def test(
        ...
        with suppress_warnings("location") as sup:
            ...
            if sys.version_info.major == 2 and sys.py3kwarning:
                ...
                sup.filter(
                    DeprecationWarning, message=r"buffer\(\) not supported in 3\.x"
                )
                sup.filter(
                    DeprecationWarning, message=r"CObject type is not supported in 3\.x"
                )
                sup.filter(
                    DeprecationWarning,
                    message=r"comparing unequal types not supported in 3\.x",
                )
