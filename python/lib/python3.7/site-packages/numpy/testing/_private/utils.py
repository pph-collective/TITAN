...
def assert_array_max_ulp(a, b, maxulp=1, dtype=None):
    ...
    if not np.all(ret <= maxulp):
        raise AssertionError("Arrays are not almost equal up to %g ULP" % maxulp)
    return ret
...
def check_free_memory(free_bytes):
    ...
    if env_value is not None:
        try:
            ...
            raise ValueError(
                "Invalid environment variable {}: {!s}".format(env_var, exc)
            )

        msg = (
            "{0} GB memory required, but environment variable "
            "NPY_AVAILABLE_MEM={1} set".format(free_bytes / 1e9, env_value)
        )
    else:
