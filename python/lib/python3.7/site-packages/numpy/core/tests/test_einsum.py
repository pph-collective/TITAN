...
class TestEinsum(object):
    def test_einsum_errors(self):
        for do_opt in [True, False]:
            ...
            assert_raises(TypeError, np.einsum, "", 0, order="W", optimize=do_opt)
    ...
    def check_einsum_sums(self, dtype, do_opt=False):
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum("i->", a, optimize=do_opt), np.sum(a, axis=-1).astype(dtype)
            )
            assert_equal(
                np.einsum(a, [0], [], optimize=do_opt), np.sum(a, axis=-1).astype(dtype)
            )
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum(a, [0, 0], optimize=do_opt), np.trace(a).astype(dtype)
            )
    ...
    def test_einsum_misc(self):
        ...
        assert_equal(
            np.einsum("...i,...i", [1, 2, 3], [2, 3, 4], optimize=u"greedy"), 20
        )
