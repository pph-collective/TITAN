...
def test_pickle_python2_python3():
    ...
    expected = np.array(
        [None, xrange, u"\u512a\u826f", b"\xe4\xb8\x8d\xe8\x89\xaf"], dtype=object
    )
...
@pytest.mark.parametrize(
    ...
    [
        np.dtype(
            np.dtype([("a", np.int8), ("b", np.int16), ("c", np.int32),], align=True),
            (3,),
        ),
        np.dtype(
            [
...
def test_version_2_0_memmap():
    ...
    ma = format.open_memmap(tf, mode="w+", dtype=d.dtype, shape=d.shape, version=(2, 0))
    ma[...] = d
    del ma
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.filterwarnings("always", "", UserWarning)
        ma = format.open_memmap(
            tf, mode="w+", dtype=d.dtype, shape=d.shape, version=None
        )
        assert_(w[0].category is UserWarning)
        ma[...] = d
        del ma
