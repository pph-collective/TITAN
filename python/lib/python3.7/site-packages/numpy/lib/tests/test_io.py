...
class TestSaveTxt(object):
    ...
    @pytest.mark.skipif(sys.platform == "win32", reason="large files cause problems")
    @pytest.mark.slow
...
class LoadTxtBase(object):
    ...
    def test_converters_decode(self):
        ...
        c.seek(0)
        x = self.loadfunc(
            c, dtype=np.unicode_, converters={0: lambda x: x.decode("UTF-8")}
        )
...
class TestLoadTxt(LoadTxtBase):
    ...
    def test_comments_unicode(self):
        ...
        c.write("# comment\n1,2,3,5\n")
        c.seek(0)
        x = np.loadtxt(c, dtype=int, delimiter=",", comments=u"#")
...
M   33  21.99
    ...
    def test_dtype_with_converters(self):
        dstr = "2009; 23; 46"
        test = np.genfromtxt(
            TextIO(dstr,), delimiter=";", dtype=float, converters={0: bytes}
        )
        control = np.array(
        ...
        assert_equal(test, control)
        test = np.genfromtxt(
            TextIO(dstr,), delimiter=";", dtype=float, converters={0: float}
        )
    ...
    def test_dtype_with_converters_and_usecols(self):
        ...
        test = np.recfromcsv(
            TextIO(dstr,), dtype=dtyp, delimiter=",", names=None, converters=conv
        )
    ...
    def test_inconsistent_dtype(self):
        ...
        kwargs = dict(
            delimiter=",", converters=converters, dtype=[(_, int) for _ in "abcde"],
        )
