...
try:
    ...
    raise ImportError(
        "You must install matplotlib (http://matplotlib.sourceforge.net/)"
    )
...
def my_erdos_renyi_binomial_random_graph(
    node_list=None, p=0.0, seed=None, directed=False
):
...
def my_barabasi_albert_graph(n, m, node_list=None, seed=None):
    ...
    if node_list is not None:
        if n != len(node_list):
            raise nx.NetworkXError(
                "node_list must have smae length as n! len(node_list)=%d,n=%d"
                % (len(node_list), n)
            )
        else:
...
class NetworkClass(PopulationClass):
    def __init__(self, N=10000, m_0=1, network_type="scale_free"):
        ...
        for agent in self.Agents:
            if (
                agent not in self.IDU_agents
                and agent not in self.NIDU_agents
                and agent not in self.MSM_agents
            ):
                self.NormalAgents.append(agent)
        self.NetworkSize = len(self.NormalAgents)
        if network_type == "scale_free":
            # scale free Albert Barabsai Graph
            self.G = my_barabasi_albert_graph(
                self.NetworkSize, m_0, node_list=self.NormalAgents
            )
    ...
    def plot_DegreeDistribution(self, graph):
        ...
        plt.axis(
            [
                min(degreeList) - (0.05 * max(degreeList)),
                max(degreeList) + (0.05 * max(degreeList)),
                0.9,
                max(degreehist) + (0.05 * max(degreehist)),
            ]
        )
        plt.show()
    ...
    def visualize_network(
        self, graph, coloring="Sex Type", pos=None, return_layout=0, node_size=None
    ):
        """
        :Purpose:
        ...
        if coloring == "Sex Type":
            ...
            Only 'Sex Type','Drug Type', and 'AIDS HIV' allowed!"
                % str(coloring)
            )
...
class TestClassMethods(unittest.TestCase):
    ...
    def test_NormalAgents(self):
        ...
        for agent in myNetworkObj.NormalAgents:
            ...
            self.assertTrue(
                agent_drug_type not in ["IDU", "NIDU"],
                "Wrong drug type!%s" % str(agent_drug_type),
            )
            self.assertTrue(
                agent_sex_type != "MSM", "Wrong sex type!%s" % str(agent_sex_type)
            )
