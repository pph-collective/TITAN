...
def my_barabasi_albert_graph(n, m, node_list=None, seed=None):
    ...
    if node_list is not None:
        if n != len(node_list):
            raise nx.NetworkXError( \
                "node_list must have smae length as n! len(node_list)={:d},n={:d}".format(len(node_list), n))
        else:
...
class NetworkClass(PopulationClass):
    ...
    def __init__(self, N=10000, m_0=1, network_type='scale_free'):
        ...
        if type(N) is not int:
            raise ValueError("Population size must be integer, n = {}, not {}".format(string(N), type(N)))
        else:
            pass
    ...
    def visualize_network(self, graph, coloring='Sex Type', pos=None,
        ...
        if coloring == 'Sex Type':
            for v in graph:
                ...
                if tmp_sextype == 'HM':
                    ...
                    raise ValueError("Check agents {} drug {}".format(v, tmp_sextype))
            ...
            Only 'Sex Type','Drug Type', and 'AIDS HIV' allowed!".format(str(coloring)))
        ...
        plt.axis('equal')
        plt.axis('off')
        filename = "Network_{:d}_{}.png".format(graph.size(), coloring)
