...
def test_select_partner_invalid(make_population, make_agent):
    ...
    assert set(bond) == {"injection", "sexual"}
    assert partner is None

    nidu_agent._SO = "HF"
    nidu_agent_sex._SO = "HF"
    ...
    partner, bond = select_partner(
        nidu_agent, empty_pop.All_agentSet, FakeRandom(0.0, 0)
    )
...
def test_select_partner_valid(make_population, make_agent):
    empty_pop = make_population()
    idu_agent = make_agent(DU="IDU")
    idu_partner = make_agent(DU="IDU")
    idu_sex_partner = make_agent(DU="IDU")
    empty_pop.add_agent_to_pop(idu_agent)
    empty_pop.add_agent_to_pop(idu_partner)
    empty_pop.add_agent_to_pop(idu_sex_partner)
    partner, bond = select_partner(
        idu_agent, empty_pop.All_agentSet, FakeRandom(0.0, 0)
    )
    assert partner is idu_sex_partner
    assert bond == ["injection"]
...
def test_sex_possible():
    ...
    assert sex_possible("HM", "WSW") == False
    assert sex_possible("HM", "MTF") == True

    assert sex_possible("MSM", "HM") == False
    assert sex_possible("MSM", "MSM") == True
    assert sex_possible("MSM", "HF") == False
    assert sex_possible("MSM", "WSW") == False
    assert sex_possible("MSM", "MTF") == True

    assert sex_possible("WSW", "HM") == False
    assert sex_possible("WSW", "MSM") == False
    assert sex_possible("WSW", "HF") == False
    assert sex_possible("WSW", "WSW") == True
    assert sex_possible("WSW", "MTF") == True

    assert sex_possible("HF", "HM") == True
    assert sex_possible("HF", "MSM") == False
    ...
    assert sex_possible("MTF", "WSW") == True
